...
import "./css/ProductsFilterness.css";
...
function ProductsFilterness() {
    ...
    if (brand){
        if(productURL.includes("?")) productURL = `${productURL}&brand=${brand}`;
        else productURL = `${productURL}?brand=${brand}`;
    }
    if (size){
        if(productURL.includes("?")) productURL = `${productURL}&size=${size}`;
        else productURL = `${productURL}?size=${size}`;
    }
    if (price){
        if(productURL.includes("?")) productURL = `${productURL}&price=${price}`;
        else productURL = `${productURL}?price=${price}`;
    }
    if (color){
        if(productURL.includes("?")) productURL = `${productURL}&color=${color}`;
        else productURL = `${productURL}?color=${color}`;
    }
    ...
    useEffect(() => {
      ...
      }, [dispatch, brand, size, price, color ]);      
  ...
  if(error) {
    ...
    return (
        ...
        <div className="productsFilterness">
...
export default ProductsFilterness
